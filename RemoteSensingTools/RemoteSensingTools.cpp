...
static CombinedBoundsResult computeCombinedBounds(const ImageInfo* left, const ImageInfo* right) {
    ...
    res.leftOffsetXf = lx;
    res.leftOffsetYf = ly;
    res.leftOffsetXi = static_cast<int>(std::round(lx));
    res.leftOffsetYi = static_cast<int>(std::round(ly));

    res.rightOffsetXf = rx;
    res.rightOffsetYf = ry;
    res.rightOffsetXi = static_cast<int>(std::round(rx));
    res.rightOffsetYi = static_cast<int>(std::round(ry));
...
int main(int argc, char** argv)
{
    ...
    if (linfo->hasGeoInfo && rinfo->hasGeoInfo) {
        ...
        // 计算相对偏移（原有）
        double offsetX = (rgt.xOrigin - lgt.xOrigin) / lgt.pixelWidth;
        double offsetY = (rgt.yOrigin - lgt.yOrigin) / lgt.pixelHeight;
        std::cout << std::fixed << std::setprecision(8);
		std::cout << "左影像相对于右影像的像素偏移: (" << offsetX << ", " << offsetY << ")\n";
		int offsetXi = static_cast<int>(std::round(offsetX));
		int offsetYi = static_cast<int>(std::round(offsetY));
        double offsetXf = static_cast<double>((offsetX - offsetXi));
        double offsetYf = static_cast<double>((offsetY - offsetYi));
		std::cout << "整数偏移: (" << offsetXi << ", " << offsetYi << ")\n";
		std::cout << "小数偏移: (" << offsetXf << ", " << offsetYf << ")\n";
		int b_width = offsetXi + rinfo->width ;
		int b_height = offsetYi + rinfo->height;
		std::cout << "重叠区域尺寸: " << b_width << " x " << b_height << "\n";
		std::cout << "重叠区域左上角在左影像中的位置: (" << offsetXi << ", " << offsetYi << ")\n";
