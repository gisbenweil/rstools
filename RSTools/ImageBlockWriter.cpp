	...
	: ds_(nullptr), width_(width), height_(height), bands_(bands), path_(path), dataType_(dataType)
{
	...
	switch (dataType_) {
	case ImageDataType::Byte:      bytesPerPixel = 1; break;
	case ImageDataType::UInt16:
	case ImageDataType::Int16:     bytesPerPixel = 2; break;
	case ImageDataType::UInt32:
	case ImageDataType::Int32:
	case ImageDataType::Float32:   bytesPerPixel = 4; break;
	case ImageDataType::Float64:   bytesPerPixel = 8; break;
	}

	uint64_t estimatedSize = static_cast<uint64_t>(width_) * height_ * bands_ * bytesPerPixel;

	if (estimatedSize > 3ULL * 1024 * 1024 * 1024) { // > ~3 GB
